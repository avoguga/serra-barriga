...
} from './JeelizFaceFilterInterfaces';
...
export class JeelizCanvas2DHelper {
  ...
  constructor(spec: IJeelizFaceFilterInitResult) {
    ...
    this.SHADERCOPY = this.build_shaderProgram(
      copyVertexShaderSource,
      copyFragmentShaderSource,
      'VIDEO'
    );
    if (this.SHADERCOPY == null) return;
    const uSampler = this.GL.getUniformLocation(
      this.SHADERCOPY,
      'samplerImage'
    );
    this.UUVTRANSFORM = this.GL.getUniformLocation(
      this.SHADERCOPY,
      'UVTransformMat2'
    );
    ...
    if (!this.GL.getShaderParameter(glShader, this.GL.COMPILE_STATUS)) {
      alert(
        'ERROR IN ' +
          typeString +
          ' SHADER: ' +
          this.GL.getShaderInfoLog(glShader)
      );
      return null;
    }
    return glShader;
  ...
  private build_shaderProgram(
    ...
    const glShaderVertex = this.compile_shader(
      shaderVertexSource,
      this.GL.VERTEX_SHADER,
      'VERTEX ' + id
    );
    const glShaderFragment = this.compile_shader(
  ...
  public draw() {
    ...
    if (
      this.CV == null ||
      this.GL == null ||
      this.SHADERCOPY == null ||
      this.VIDEOTEXTURETRANSFORMMAT2 == null ||
      this.CANVAS2D == null
    )
      return;
    ...
    this.GL.uniformMatrix2fv(
      this.UUVTRANSFORM,
      false,
      this.VIDEOTEXTURETRANSFORMMAT2
    );
  ...
  public getCoordinates(detectedState: IJeelizFaceFilterDetectState) {
    ...
    this.COORDINATES.x = Math.round(
      (0.5 + 0.5 * detectedState.x - 0.5 * detectedState.s) * this.CV.width
    );
    this.COORDINATES.y = Math.round(
      (0.5 + 0.5 * detectedState.y - 0.5 * detectedState.s) * this.CV.height
    );
